...
export class tailHealerJob extends Task_Role {
  ...
  loop_task(): void {
    ...
    if (myTailers.length < 6) {
      const idleTailers = myTailers.filter(
    ...
    if (!this.head) {
      const head_rtn = <Cre_battle | undefined>(
        friends.find(
          i =>
      ...
      if (head_rtn) {
        ...
        //no head
        const myGroup = getGroup_all(this.master);
