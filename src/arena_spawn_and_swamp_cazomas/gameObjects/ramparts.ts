...
import { moveMap } from "../deprecated/maps";
...
export function enemyRampartIsHealthy(
  ram: StructureRampart,
  print: boolean = false
) {
...
export function rampartIsHealthy(
  ...
  if (hasOppoUnitsAround(ram, 3)) {
    ...
    const RANum = sum(around3Enemies, i =>
      i.getHealthyBodiesNum(RANGED_ATTACK)
    );
    ...
    if (useExtra) {
      return (
        hits(ram) >= rampartHealthBias0 + relu(extraBias - extraBiasSafeReduce)
      );
    } else {
...
export function gotoTargetRampart(cre: Cre, targetRampart: Pos) {
  ...
  let sRtn = getDecideSearchRtnNoArea(cre, targetRampart, {
      ...
      if (!inMyHealthyRampart(firstPos) || cost > 50) {
        ...
        const tarFriends = getOtherFriends(cre).filter(
          i =>
            ...
            !(cre.getBodiesNum(WORK) > 0 && cre.macro
              ? cre.macro.getIsWorking()
              : false) &&
            i.getHealthyBodiesNum(ATTACK) < cre.getHealthyBodiesNum(ATTACK)
        );
