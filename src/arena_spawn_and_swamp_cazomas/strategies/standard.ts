...
import {
  ...
  enemyHealReduction,
  enemyMoveSpeedBonus,
  enemyMoveSpeedReduce,
  enemyQuickAttackReduce,
  enemyRABonus,
  enemyRAReduce,
  enemyRASlowBonus,
  enemySlowShoterBonus,
  enemyWorkerBonus,
  getSuperior,
  getSuperiorRate,
  spawnBlockBonus,
  spawnDangerousBonus,
  spawnEnergyAroundBonus,
  spawnEnergyBonus,
  ticksBonus,
  ticksReduce,
  totalInferiorityBonus,
  totalSuperiorityBonus,
  totalSuperiorityRateBonus,
...
function getMainArmyNum(fri: Cre[]): number {
  ...
  const rtn: number =
    0.5 * sumForceByArr(fri.filter(i => i.getBodiesNum(ATTACK) > 0));
  // PS("mainArmyNum=" + rtn)
  return rtn;
}
...
export function getSpawnTypeList_st(): SpawnType[] {
  ...
  const defenderBonus =
    friends.filter(i => i.role === defender_rampart).length >= 3 ? 0 : 1;
  //
  SAN(
    displayPos(),
    "spawnAndExtEn",
    spawnAndExtensionsEnergy(spawn, true, true)
  );
  ...
  const spawnTypeList: SpawnType[] = [
    ...
    new SpawnType(
      jamer,
      6 * EBB1p7 * TSRR * TKB6_3 * EEB(50, 1.2),
      TB("M"),
    ...
    new SpawnType(
      stdAttacker,
      TKB4_6 * supplyBonus * EEB(980, 6),
      TB("10M6A"),
      fri => getMainArmyNum(fri)
    ),
    //850
    new SpawnType(stdAttacker, supplyBonus * EEB(850, 3.5), TB("9M5A"), fri =>
      getMainArmyNum(fri)
    ),
    //670
    new SpawnType(stdAttacker, supplyBonus * EEB(670, 3), TB("7M4A"), fri =>
      getMainArmyNum(fri)
    ),
    //490
    new SpawnType(stdAttacker, supplyBonus * EEB(490, 2.8), TB("5M3A"), fri =>
      getMainArmyNum(fri)
    ),
    //410
    new SpawnType(stdAttacker, supplyBonus * EEB(410, 2.5), TB("5M2A"), fri =>
      getMainArmyNum(fri)
    ),
    //360
    new SpawnType(stdAttacker, supplyBonus * EEB(360, 2), TB("4M2A"), fri =>
      getMainArmyNum(fri)
    ),
    //230
    new SpawnType(
      stdAttacker,
      supplyBonus * TSRR * EEB(230, 1.4),
      TB("3MA"),
      fri => getMainArmyNum(fri)
    ),
    //180
    ...
    new SpawnType(
      stdAttacker,
      1.1 * E5MAB * EWB1p5 * ERB1p5 * ESSB2 * EEB(330, 2),
      TB("5MA"),
      fri =>
        fri.filter(
          i =>
    ...
    new SpawnType(
      stdAttacker,
      1.2 * TKB4_6 * ESSB5 * EEB(990, 6),
      TB("15M3A"),
      fri =>
        0.8 * getMainArmyNum(fri) +
        0.2 *
          fri.filter(i => i.role === stdAttacker && i.getSpeed_general() === 1)
            .length
    ),
    // //960
    ...
    new SpawnType(
      toughDefender,
      0.8 * TIB * SDB * defenderBonus * EEB(510, 2.5),
      TB("18T3AM"),
    ...
    new SpawnType(
      defender_rampart,
      0.25 * ERB4 * TIB * SDB * defenderBonus * EEB(200, 2),
      TB("RM"),
      fri => fri.filter(i => i.role === defender_rampart).length
    ),
    //500
    ...
    new SpawnType(
      builderStandard,
      1.6 * TSB * TKR2_6 * TSRB * EEB(350, 1.5),
      TB("C3MCW"),
      fri => fri.filter(i => isBuilderOutSide(i.role)).length
    ),
    //300
    new SpawnType(
      ...
      TB("5CM"),
      fri =>
        fri.filter(i => i.role === harvester && i.getBodiesNum(CARRY) >= 2)
          .length
    ),
    //200
    new SpawnType(
    ...
    new SpawnType(
      stdHealer,
      1.2 * EAB1p1 * TKB2_2 * TKB4_3 * ERB2 * EEB(500, 3.5),
      TB("5MH"),
      fri => fri.filter(i => i.role === stdHealer).length
    ),
    //400
    new SpawnType(
      stdHealer,
      1 * EAB1p1 * TKB2_2 * TKB4_3 * ESSB2 * ERB2 * EEB(400, 2),
      TB("3MH"),
      fri => fri.filter(i => i.role === stdHealer).length
    ),
    // //400+450
