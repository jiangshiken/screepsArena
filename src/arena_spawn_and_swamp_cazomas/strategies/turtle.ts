...
import { createCS, createCS_wait, supplyCS } from "../constructionSite"
import { set_setMiniForceMapOn } from "../maps"
import { baseLoseRampart } from "../ramparts"
...
import { getSpawnAndBaseContainerEnergy, resetStartGateAvoidFromEnemies, spawn, spawnCleared, spawnCreep, spawnCreepInFront } from "../spawn"
import { SpawnType } from "../spawnTypeList"
import { supplyHarvester } from "../strategy_tool"
import { displayPos } from "../util_attackable"
import { TB } from "../util_autoBodys"
import { set_switchCPUModeOn } from "../util_CPU"
import { friends, getEnemyArmies, getFreeEnergy, setIsTurtleContainer } from "../util_Cre"
import { tick } from "../util_game"
import { containers, isMyGO, myConstructionSites } from "../util_gameObjectInitialize"
...
export function useTurtleStrategy() {
	...
	const unProtectPoss = tarProtectPoss.filter(i => !findGO(i, StructureRampart) && !findGO(i, StructureWall))
...
export function supplyWalls(tarProtectPoss: Pos[]) {
	...
	for (const pos of tarProtectPoss) {
		if (!overallMap.get(pos).find(i => i instanceof ConstructionSite && isMyGO(i))) {
...
export function supplyBuilders() {
	if (spawnCleared(spawn) && friends.find(i => i.role === builderTurtle) === undefined) {
		if (currentGuessPlayer === Dooms) {
...
function supplyRamparts() {
	...
	const rangePos = getRangePoss(spawn, 1).filter(i => !findGO(i, StructureWall))
