...
export function validRes(res: Res) {
	let dropEvents = filterEventList(tick - 1, tick + 1,
		i => i instanceof ResourceDropEvent
			&& atPos(i, res)
	)
...
export let enemySpawnPos: StructureSpawn = getObjectsByPrototype(StructureSpawn)[0]
